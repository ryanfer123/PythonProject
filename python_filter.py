import cv2
import numpy


#Chunk 2 - Loading the model and importing image
face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml")
sunglasses = cv2.imread(r"/Users/ryanfernandes/Desktop/Pythonfilterproject/sunglasses.png", cv2.IMREAD_UNCHANGED)

#Chunk 3 - Vid Capture 
cap = cv2.VideoCapture(0)

while True:
    ret, frame = cap.read()
    if not ret: break

    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    faces = face_cascade.detectMultiScale(gray, 1.3, 5)
    
    for (x,y,w,h) in faces:
        gw = w
        gh = int(sunglasses.shape[0]* gw/ sunglasses.shape[1])
        resized = cv2.resize(sunglasses,(gw,gh))

        y1 = y + int(h*0.01)
        y2 = y1 + gh
        x1 = x
        x2 = x + gw

        alpha = resized[:,:,3]/255.0
        for c in range(3): 
            frame[y1:y2, x1:x2, c] = (
                alpha * resized [:,:,c]+ (1-alpha)*frame[y1:y2, x1:x2,c]
            )

    cv2.imshow("sunglasses", frame)
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break

cap.release
cv2.destroyAllWindows()
